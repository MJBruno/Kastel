...
pub fn build_signature_help(workspace: &Workspace, uri: &str, line: u32, character: u32) -> Option<Value> {
    let document = workspace.get(uri)?;
    let offset = line_and_byte_to_offset(&document.text, line as usize, utf16_byte_position(line_text_at(&document.text, line as usize), character as usize));
    let call = find_active_call(&document.text, offset)?;
    let mut candidates = Vec::new();

    if let Some((name, _, doc)) = BUILTIN_FUNCTIONS.iter().find(|(name, ..)| *name == call.callee) {
        let label = BUILTIN_FUNCTIONS.iter().find(|(n,_,_)| *n == call.callee).map(|(_,s,_)| s.to_string()).unwrap_or_else(|| format!("{}(...)", name));
        let params = parameter_labels(&label);
        candidates.push(Candidate { label, params, documentation: doc.to_string() });
    }

    if candidates.is_empty() {
        if let Some(signatures) = document.types.function_signatures(&call.callee) {
            candidates.extend(signatures.iter().map(|s| candidate_from_function_signature(s)));
        }
    }
    ...
    if candidates.is_empty() {
        if let Some((receiver, member)) = split_member_callee(&call.callee) {
            ...
            for ty in infer_receiver_types(workspace, uri, document, receiver, receiver_offset) {
                candidates.extend(member_signature_candidates(workspace, uri, document, &ty, member));
            }
        }
    }
...
fn candidate_from_function_signature(signature: &crate::type_info::FunctionSignature) -> Candidate {
    Candidate {
        ...
        params: signature.params.iter().map(|(name, ty)| format!("{}: {}", name, ty)).collect(),
...
fn type_call_candidates(document: &WorkspaceDocument, ty: &Type) -> Vec<Candidate> {
    match ty {
        ...
        Type::Overloads(overloads) => overloads.iter().map(|f| candidate_from_function_type("call", f)).collect(),
        Type::Named(name) => document.classes
            .all_methods(name)
            .iter()
            .filter(|method| method.name == "initialize")
            .map(|method| Candidate {
                label: method.signature(),
                params: method.params.iter().enumerate().map(|(i,n)| method.param_types.get(i).and_then(|t| t.as_ref()).map(|t| format!("{}: {}", n, crate::class_index::type_expr_display(t))).unwrap_or_else(|| n.clone())).collect(),
...
fn member_signature_candidates(
    ...
    match ty {
        ...
        Type::Named(class_name) => document.classes
...
fn module_signature_candidates(workspace: &Workspace, uri: &str, module_path: &str, member: &str) -> Vec<Candidate> {
    ...
    if let Some(signatures) = module.types.function_signatures(member) {
        return signatures.iter().map(candidate_from_function_signature).collect();
    }
...
fn method_candidate(method: &MethodInfo) -> Candidate {
    Candidate {
        label: method.signature(),
        params: method.params.iter().enumerate().map(|(i, name)| method.param_types.get(i).and_then(|t| t.as_ref()).map(|t| format!("{}: {}", name, crate::class_index::type_expr_display(t))).unwrap_or_else(|| name.clone())).collect(),
        documentation: "Méthode de classe".to_string(),
    }
}

fn table_signature(table: &[(&str, &str, &str)], member: &str) -> Vec<Candidate> {
    table.iter().find(|(name, ..)| *name == member).map(|(_, signature, doc)| vec![Candidate {
...
fn candidate_from_function_type(name: &str, function: &FunctionType) -> Candidate {
    let params = function.params.iter().enumerate().map(|(i, ty)| format!("arg{}: {}", i + 1, ty)).collect::<Vec<_>>();
    Candidate {
        label: format!("{}({}) -> {}", name, params.join(", "), function.return_type),
        params,
        documentation: "Fonction Kastel".to_string(),
    }
}
...
fn find_active_call(source: &str, offset: usize) -> Option<CallContext> {
    ...
    for index in (0..offset.min(bytes.len())).rev() {
        match bytes[index] {
            b')' | b']' | b'}' => depth += 1,
            b'(' => {
                if depth == 0 { open = Some(index); break; }
                depth -= 1;
            }
...
fn parameter_labels(signature: &str) -> Vec<String> {
    let open = match signature.find('(') { Some(v) => v, None => return Vec::new() };
    let close = match signature.rfind(')') { Some(v) if v > open => v, _ => return Vec::new() };
    let inner = &signature[open + 1..close];
    split_top_level(inner).into_iter().map(|p| p.trim().to_string()).filter(|p| !p.is_empty()).collect()
}
...
struct CallContext { open_paren: usize, callee: String }
...
mod tests {
    ...
    fn signature_help_uses_active_parameter() {
        ...
        ws.open("file:///main.ks".to_string(), 1, "func add(a: int, b: int) -> int { return a + b }\nadd(1, ".to_string());
        let line = 1;
        let character = 7;
